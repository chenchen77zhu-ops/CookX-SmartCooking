<template>
 <aside class="local-test-panel" aria-label="本地测试工具">
  <details><summary>本地测试版 · 无需服务器 · 测试工具</summary>
   ...
   <button :disabled="busy" @click="recognize">载入识别示例</button><button :disabled="busy" @click="router.push('/home?tab=Recipes')">进入烹饪演练</button>
...
<style scoped>
.local-test-panel { position: fixed; inset: var(--safe-area-inset-top, env(safe-area-inset-top, 0px)) var(--sar) auto var(--sal); z-index: 1900; color: var(--ck-warn-text); background: var(--ck-bg-raised); font-size: 12px; border-bottom: 1px solid var(--ck-hairline); overflow-wrap: anywhere; }
.local-test-panel summary { cursor: pointer; font-weight: 600; height: 32px; line-height: 32px; padding: 0 16px; }
.local-test-panel details[open] { max-height: calc(90dvh - var(--sat) - var(--sab)); overflow-y: auto; box-shadow: var(--ck-shadow-strong); }
.local-test-panel button, .local-test-panel select { min-height: 40px; max-width: 100%; border: 1px solid var(--ck-input-border); color: var(--ck-text); background: var(--ck-input-bg); border-radius: 8px; padding: 7px; margin: 4px; font: inherit; }
.local-test-panel label { display: inline-block; padding: 0 12px; }
.local-test-panel p { line-height: 1.6; padding: 0 16px; }
</style>
