  ...
  <section class="freshness-card" @click.stop aria-label="鲜度评估">
    ...
    <template v-else>
      <div class="fc-head">
        ...
        <p class="fc-score">FreshScore：{{ hasScore(detail.fresh_score) ? detail.fresh_score : '数据不足' }} <span>{{ detail.freshness_label }}</span></p>
      </div>
      <p class="fc-meta">置信度：{{ hasScore(detail.confidence_score) ? `${Math.round(detail.confidence_score * 100)}%` : '数据不足' }} <span>{{ detail.confidence_level }}</span></p>
      <p class="fc-meta">风险：{{ detail.risk_flags?.length ? detail.risk_flags.join('；') : '未返回风险标记（不代表安全）' }}</p>
      <ul v-if="detail.reasons?.length" class="fc-reasons"><li v-for="reason in detail.reasons" :key="reason">{{ reason }}</li></ul>
      <details class="freshness-details">
        ...
        <p>{{ LOCAL_TEST_MODE ? '此处分项与权重为固定测试样例；没有执行 FreshFusion。' : '权重和分项直接来自服务端；未提供的视觉与历史数据不作推测。' }}</p>
...
.fc-meta { color: var(--ck-text-3); }
.fc-reasons { margin: 4px 0; color: var(--ck-text-2); }
.freshness-details { margin-top: 4px; border-top: 1px solid var(--ck-hairline); }
.freshness-details dl { margin-top: 8px; }
summary { padding: 8px 0; color: var(--ck-text-2); font-weight: 600; cursor: pointer; } dt { color: var(--ck-text); font-weight: 600; } dd { margin: 0 0 8px; }
p { margin: 4px 0; } ul { padding-left: 18px; } small, .disclaimer { color: var(--ck-text-3); font-size: 12px; }
</style>
