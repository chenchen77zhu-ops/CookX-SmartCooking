<template>
 <main :class="['business-page', { 'is-tab-page': !onlyFavorites }]"><header><button v-if="onlyFavorites" @click="router.push('/profile')" aria-label="返回我的">‹</button><h1>{{ onlyFavorites?'我的收藏':'菜谱复刻' }}</h1></header><p class="note">复刻会创建独立副本并保留来源。核对食材后送到烹饪页，由你决定何时开始；不会直接计时或扣库存。</p>
