<template>
 <main class="business-page"><header><button @click="router.push(onlyFavorites ? '/profile' : { path: '/home', query: { tab: 'Recipes' } })" :aria-label="onlyFavorites ? '返回我的' : '返回 AI 菜谱'">‹</button><h1>{{ onlyFavorites?'我的收藏':'菜谱复刻' }}</h1></header><p class="note">复刻会创建独立副本并保留来源。核对食材后送到烹饪页，由你决定何时开始；不会直接计时或扣库存。</p>
...
function prepare(){try{const copy=checked.value.copy;queueRecipeDraft(readUserId(),{...copy.recipe,cookx_copy_id:copy.id,source_meta:copy.source,copy_version:copy.version});router.push('/home?tab=Recipes')}catch(e){error.value=e.message}}
