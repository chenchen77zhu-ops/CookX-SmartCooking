<template>
 ...
 <section><div class="actions"><button :disabled="busy" @click="mine=false;hot=false;load()">大家的作品</button><button :disabled="busy" @click="mine=true;hot=false;load()">我的作品</button><button :disabled="busy" @click="mine=false;hot=true;load()">近七天热门</button><button :disabled="busy" @click="load">刷新作品</button></div></section>
 <p v-if="hot" class="note">{{ rankRule }}</p><section v-for="post in posts" :key="post.id" class="post-card"><b>{{ post.author_name }}</b><p v-if="post.hot" class="note">近七天点赞 {{ post.hot.likes }} 人 · 评论 {{ post.hot.commenters }} 人</p><p class="note">{{ new Date(post.created_at).toLocaleString() }}<span v-if="post.hidden"> · 已隐藏：{{ post.moderation_reason }}</span></p><p style="white-space:pre-wrap">{{ post.text }}</p><PrivateImage v-for="id in post.image_ids" :key="id" :id="id" /><div v-if="post.recipe"><h2>{{ post.recipe.dish_name }}</h2><p class="note">{{ post.recipe.steps.length }} 个结构化步骤</p><button v-if="!post.hidden" :disabled="busy||!!pending" @click="replicate(post)">复刻关联菜谱</button></div><div class="actions"><template v-if="post.owner===user.id"><button :disabled="busy||!!pending" @click="edit(post)">编辑作品</button><button :disabled="busy||!!pending" @click="remove(post)">删除作品</button></template><button v-if="!post.hidden" :disabled="busy||!!pending" @click="report(post)">举报内容</button><button v-if="user.role==='admin'" :disabled="busy||!!pending" @click="moderate(post,!post.hidden)">{{ post.hidden?'恢复展示':'隐藏内容' }}</button></div><PostLikes v-if="!post.hidden" :post-id="post.id" /><PostComments v-if="!post.hidden" :post-id="post.id" /></section>
...
const router=useRouter(),user=JSON.parse(localStorage.getItem('user')||'{}'),posts=ref([]),copies=ref([]),mine=ref(false),hot=ref(false),rankRule=ref(''),text=ref(''),images=ref([]),copyId=ref(''),uploading=ref(false),editing=ref(null),reports=ref([]),hiddenPosts=ref([])
...
let loadRevision=0
async function load(){const version=++loadRevision;posts.value=[];try{const [result,sources]=await Promise.all([businessApi(hot.value?'/community/hot':'/community/posts?mine='+mine.value),businessApi('/recipes/sources?kind=copy')]);if(version!==loadRevision)return;rankRule.value=result.rule||'';posts.value=result.items;copies.value=sources.items}catch(e){if(version===loadRevision)error.value=apiError(e)}}
