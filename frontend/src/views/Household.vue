...
let detailRevision=0,personalRevision=0
async function loadDetail(){const version=++detailRevision,family=selected.value;detail.value=null;inviteCode.value='';resetStock();if(!family)return;try{const result=await businessApi(`/households/${family}`);if(version===detailRevision&&selected.value===family)detail.value=result}catch(e){if(version===detailRevision)error.value=apiError(e)}}
async function loadPersonal(){const version=++personalRevision;try{const result=await businessApi('/personal-inventory');if(version===personalRevision)personal.value=result}catch(e){if(version===personalRevision)error.value=apiError(e)}}
