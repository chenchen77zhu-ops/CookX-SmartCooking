    ...
    <main v-else class="ck-page home">
      ...
      <section class="ck-card today">
        ...
        <button type="button" class="today__row" @click="openRecipes">
          ...
          <span class="today__copy">
            <b>{{ LOCAL_TEST_MODE ? '本地预置操作演练' : '用冰箱食材做一道菜' }}</b>
            <span class="today__meta"><span><CkIcon name="sparkle" :size="15" />{{ LOCAL_TEST_MODE ? '固定示例 · 非 AI 生成' : 'AI 智能推荐' }}</span></span>
          </span>
      ...
      <button type="button" :class="['expiry', { 'is-error': inventoryError }]" @click="inventoryError ? fetchInventory() : openInventory()">
        ...
        <span class="expiry__copy">
          ...
          <template v-else-if="expiringNames.length"><b>临期食材提醒</b><small><em>{{ expiringNames.join('、') }}</em> {{ LOCAL_TEST_MODE ? '示例临期项，不作食用建议' : '需要尽快食用' }}</small></template>
