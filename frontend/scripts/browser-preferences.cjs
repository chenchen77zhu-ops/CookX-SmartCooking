const {chromium}=require(process.env.COOKX_PLAYWRIGHT||'playwright'),assert=require('node:assert/strict')
;(async()=>{const browser=await chromium.launch({headless:true,...(process.env.COOKX_CHROME?{executablePath:process.env.COOKX_CHROME}:{})});try{
 const page=await browser.newPage();await page.addInitScript(()=>{if(localStorage.getItem('user'))return;localStorage.setItem('user',JSON.stringify({id:'a'}));localStorage.setItem('cookx_preferences',JSON.stringify({dislikedIngredients:'旧设备花生'}));localStorage.setItem('cookx:preferences:v1:a',JSON.stringify({dislikedIngredients:'账号A香菜'}))})
