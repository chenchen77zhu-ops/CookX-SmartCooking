...
;(async()=>{
 ...
 try {
  ...
  await page.goto((process.env.COOKX_BASE_URL||'http://127.0.0.1:4173')+'/#/home?tab=Recipes')
  ...
  await page.getByRole('button',{name:'开始指导',exact:true}).click();await page.getByRole('button',{name:'烹饪工具',exact:true}).click()
  await page.getByText('时长未提供',{exact:true}).waitFor()
  await page.getByRole('button',{name:'下一步',exact:true}).click()
  await page.getByText('热锅',{exact:true}).first().waitFor()
  await page.waitForFunction(()=>document.querySelector('.c-eta')?.innerText.includes('00:59'))
  ...
  await page.getByRole('button',{name:/^CookX Sense/}).click()
