...
;(async () => {
 ...
 try {
  ...
  await page.goto((process.env.COOKX_BASE_URL || 'http://127.0.0.1:4173')+'/#/fridge')
  const card=page.locator('.food-card').filter({has:page.getByRole('heading',{name:'鸡蛋',exact:true})})
  const other=page.locator('.food-card').filter({hasNot:page.getByRole('heading',{name:'鸡蛋',exact:true})})
  await other.locator('.food-card-top').click()
  await page.getByText('FreshScore：数据不足',{exact:false}).waitFor()
  await card.locator('.food-card-top').click()
  ...
  if(await card.locator('.food-card-top').getAttribute('aria-expanded')!=='true') await card.locator('.food-card-top').click()
  await card.getByRole('button',{name:'编辑食材'}).click()
  await page.getByText(/只保存修改过的字段/).waitFor()
