...
| B4 页面闭环 | feat/p2-inventory-flow | 7be8cf7 (B3) | 待联调（已支持路径通过） |
...
B4：按用户保存识别草稿及缺失信息；入库后重读鲜度；请求过推荐时自动更新，失败不撤销库存。40 项前端测试、生产构建、移动浏览器识别确认闭环、推荐故障恢复、用户隔离与写入响应丢失防重复验证通过。识别为测试替身，库存/FreshFusion/推荐使用隔离真实后端。
