...
**软件通过**：398 项后端测试、71 项前端测试、3 项温度研究测试；22 组浏览器脚本（20 组全量流程＋后端地址设置＋私人头像）分别通过，最新相关菜单/完整流程再次通过。JDK21 / SDK36 assembleDebug 和 testDebugUnitTest 通过。首份集成 APK 源码提交为 5af0f6d；1.3.0 发布包的精确提交与校验值见附件 build-manifest.json。编译机器为 Windows PC，没有连接手机。
