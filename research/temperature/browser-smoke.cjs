...
;(async()=>{
 ...
 await page.route('**/api/**',async route=>{
   if(route.request().url().includes('/tts?')) return route.fulfill({json:{audio_url:'/api/test-temperature-audio.wav'}})
 ...
 assert.ok(!(await page.locator('.current-step-card .panel-heading').innerText()).includes('00:30'),'step timer must continue during inference')
 ...
 const report={mobileWidth:390,overflow,uncaughtErrors:errors,exportSamples:exported.samples.length,
   checks:['model-load','wasm-inference','simulation-source','export','probe-correction','step-navigation','model-failure-fallback','audio-pause-resume','timer-during-inference'],
